...
public class proxy : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        string token = "8dd61dd8ad7c4d59608abb21bd1edb9";
        if (!String.IsNullOrEmpty(token))
        {
            if (uri.StartsWith("http://ditu.zj.gov.cn") && !uri.Contains("token="))
            {
                if (uri.Contains("?"))
                    uri += "&token=" + token;
                else
        ...
        //req.Headers.Add("Host:" + "ditu.zj.gov.cn");
